...
tmp = load(fullfile(sessionAnalysisFolder, sprintf('trial_%d_nvt.mat', iTrial)));
trial = tmp.trial;
...
subplot(p,q,k)
k = k + 1;
isInterior = arena.inInterior(x_cm, y_cm);
plot(x_px(isInterior), y_px(isInterior), 'k-')
hold on
plot(x_px(~isInterior), y_px(~isInterior), 'ro', 'markersize', 2, 'markerfacecolor', 'r')
