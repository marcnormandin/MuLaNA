function mltp_plot_bfo_general_placey_session_grouped(obj, session, rotDeg)
    % This function is used by the pipeline and supplied with either
    % rotDeg = 90 or 180. It plots the groups best fit orientations.
    
    dataFilename = fullfile(session.getAnalysisDirectory(), obj.Config.best_fit_orientations_placey.outputFolder, ...
        sprintf('bfo_%d_placey_percell.mat', rotDeg));
    ...
    for iGroup = 1:numGroups
        ...
        for iCell = 1:numCells
            prob(iCell,:) = perCell(iCell).(sprintf('prob_%s', group));
        end
        groupMean(iGroup,:) = mean(prob,1, 'omitnan');
        groupStd(iGroup,:) = std(prob,0,1, 'omitnan');
    end
    ...
    prefix = sprintf('bfo_%d_placey_grouped', rotDeg);
    ...
    save( fullfile(outputFolder, sprintf('bfo_%d_placey_grouped.mat', rotDeg)), 'numGroups', 'groupMean', 'groupStd', 'groups', 'rotDeg', ...
