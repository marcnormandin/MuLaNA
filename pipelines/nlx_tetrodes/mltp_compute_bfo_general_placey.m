function [perCell, total] = mltp_compute_bfo_general_placey(obj, session, rotDeg, mirrorContexts)
    ...
    outputFolderPValue = fullfile(session.getAnalysisDirectory(), obj.Config.placemaps.outputFolder);
    ...
    threshold = obj.Config.best_fit_orientations_placey.criteria_pvalue_threshold;
    ...
    for iCell = 1:numCells
	    ...
	    % needed for the pvalue
	    x1p = load(fullfile(outputFolderPValue, fnames{iMap1}));
            if x1p.mltetrodeplacemap.informationRate_pvalue > threshold
		continue;
	    end
            ...
            % needed for the pvalue
	    x2p = load(fullfile(outputFolderPValue, fnames{iMap2}));
	    if x2p.mltetrodeplacemap.informationRate_pvalue > threshold
		continue;
	    end
