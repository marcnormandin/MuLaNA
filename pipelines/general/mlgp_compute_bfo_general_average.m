function mlgp_compute_bfo_general_average(obj, rotDeg, group)
    ...
    bfo_prob_std = std(bfo_session_prob_mean, 0, 1); % ./ sqrt(size(bfo_session_prob_mean,1));
    
    bfo_corr_avg = mean(bfo_session_corr, 1);
    bfo_corr_std = std(bfo_session_corr, 0, 1); % ./ sqrt(size(bfo_session_corr,1));
