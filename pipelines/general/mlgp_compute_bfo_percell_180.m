...
%     placemapsInclude = tmp.inclusionData.include; % should check that ids match the maps
    ...
    save(fullfile(session.getAnalysisDirectory(), sprintf('%s_bfo_percell_180.mat', session.getName())), 'mapType', 'rotationsDeg', 'perCell', 'animalName', 'sessionName', 'numCells', 'uniqueCellIds', 'numTrials', 'numContexts');
end % function
