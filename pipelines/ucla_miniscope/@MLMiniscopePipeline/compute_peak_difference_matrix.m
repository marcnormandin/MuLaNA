function compute_peak_difference_matrix(obj, session)
...
%     numCells = length(unique(cellIds)); 
    ...
    % Save the data
    outputMatFilename = fullfile(sessionAnalysisFolder, sprintf('%s_compute_peak_difference_matrix_%s.mat', sessionName, mapNameToUse));
    ...
    title(sprintf('Peak Rate Difference\naveraged across (%d) cells', numUniqueCells), 'interpreter', 'none')
    ...
    outputFigFilename = fullfile(sessionAnalysisFolder, sprintf('%s_compute_peak_difference_matrix.fig', sessionName, mapNameToUse));
    savefig(hFig, outputFigFilename);
    
    outputSvgFilename = fullfile(sessionAnalysisFolder, sprintf('%s_compute_peak_difference_matrix.svg', sessionName, mapNameToUse));
