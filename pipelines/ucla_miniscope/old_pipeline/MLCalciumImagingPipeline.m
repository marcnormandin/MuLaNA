classdef MLCalciumImagingPipeline < handle
    ...
    properties
        ...
        availablePerTrialTasks = { ...
            ...
            'plot_allocentric_placemaps_rectangle', ...
            'pipeline_dummy_test'
        };
    ...
    methods
        ...
        function executePerTrialTaskByIndex( obj, task, iSession, iTrial, varargin )
            if obj.isValidPerTrialTask( task )
                ...
                elseif strcmp(task, 'plot_allocentric_placemaps_rectangle')
                    ml_cai_pipeline_plot_allocentric_placemaps_rectangle(obj, trial);
    ...
    methods %(Access = private)
        ...
        function [experiment] = core_pipeline_init( obj, experimentParentFolder, analysisParentFolder )
            ...
            for iSession = 1:numSessions
                ...
                % We want to find only the trials that are desired to be
                % used
                useIndices = find(exp.session{iSession}.trial_info.use == 1);
                numTrials = length(useIndices);
                trialFolders = {exp.session{iSession}.trial_info.folder{useIndices}};
                % trialFolders is a cell array so we need to access the
                % folder names as trialFolders{iTrial}
                
                % also get the meta information
                ti = exp.session{iSession}.trial_info;
                metaSequenceNums = ti.sequence_num(useIndices);
                metaContexts = ti.contexts(useIndices);
                metaDigs = ti.digs(useIndices);
                ...
                for iTrial = 1:numTrials
                    tfname = trialFolders{iTrial}; % eg 'H##_M##_S##'
                    
                    fprintf('Session %d -> Trial %d -> %s\n', iSession, iTrial, tfname);

                    
                    %trial.rawFolder = [sessionFolder filesep trialFolders(iTrial).name];
                    trial.rawFolder = [sessionFolder filesep tfname];

                    trial.analysisFolder = [analysisParentFolder filesep session{iSession}.name filesep tfname];
                    trial.timeString = tfname;

                    s = split(tfname, filesep);
                    ...
                    trial.context = metaContexts(iTrial);
                    trial.sequenceNum = metaSequenceNums(iTrial);
                    trial.digs = metaDigs{iTrial};
                    
                    session{iSession}.trial{iTrial} = trial;
                    
                    % Create the analysis folder if it doesn't already exist
                    if ~isfolder(trial.analysisFolder)
                        if VERBOSE
                            fprintf('Creating analysis trial folder (%s) ... ', trial.analysisFolder);
                        end
                        mkdir(trial.analysisFolder);
                        if VERBOSE
                            fprintf('done!\n');
                        end
                    end
        ...
        function interp_behaviour_to_scope_time_video_coordinates_two_led( obj, trial )
            ...
            % Now filter by the quality of the frames
            % 4, 10, 75
            quality = ml_cai_quality_all_two_led(behavTime, behavPosLed1, behavPosLed2, 'differenceFactor', obj.config.behaviour_camera.quality.difference_factor, 'separationMin', obj.config.behaviour_camera.quality.separation_min, 'separationMax', obj.config.behaviour_camera.quality.separation_max);
...
%                 boundsy = [min(y), max(y)];
        ...
        %% Plot the allocentric placemaps of the arenas mapped to a rectangle where the feature is at the top
        function ml_cai_pipeline_plot_allocentric_placemaps_rectangle(obj, trial)
            trialFolder = trial.analysisFolder;
            outputFolder = fullfile(trialFolder, obj.config.allocentric_placemaps_rectangle.output_folder);
            ...
            mapsubfolder = obj.config.allocentric_placemaps_rectangle.output_folder;

            h = figure;
            p = 2; q = 5; k = 1;
            cellsPerFigure = 5;
            trialCellNumsToProcess = 1:numNeurons;
            numCellsToProcess = length(trialCellNumsToProcess);
            numFigures = 1;
            for iCell = 1:numCellsToProcess
                cellNum = trialCellNumsToProcess(iCell);
                
                % Load the data
                fn = sprintf('cell_%d_mlcalciumallocentricplacemaprectangle.mat', cellNum);
                ffn = fullfile(trialFolder, mapsubfolder, fn);
                tmp = load(ffn);
                ...
                title(sprintf('Cell %d', tmp.trialCellNum))
                ...
                if k == cellsPerFigure && iCell < numCellsToProcess
                    saveas(h, fullfile(outputFolder, sprintf('pms_%d.png', numFigures)), 'png');
                    close(h);
        ...
        %%
        function ml_cai_pipeline_analysis_cellreg_correlation_vs_orientation(obj, session, varargin)
            varargin = varargin{1};

            disp(varargin)
            fprintf('Length of varargin = %d\n', length(varargin));
            if length(varargin) ~= 1
                celldisp(varargin);
                error('Function called with incorrect arguments.\n');
            else
                x = varargin{1};
                if ~isempty(x)
                    x = x(1);
                ...
                if ~isnumeric(x) && ~isempty(x)
                    error('Single input should be a number representing the context id or empty to use all of them.');
                end
                context_id = x;
            end
            ...
            numRegisteredTrials = size(cell_to_index_map,2);
            numSessionTrials = session.numTrials;
            
            if numRegisteredTrials ~= session.numTrials
                error('The number of registered trials (%d) does not match the actual number of session trials (%d).\n', numRegisteredTrials, numSessionTrials);
            end
            ...
            if isempty(context_id)
                ...
                fprintf('Correlating only with context (%d).\n', context_id);
            end
            ...
            for iCell = 1:numCells
                ...
                if isempty(context_id)
                    matchingTrials = find(cellTrialIndices ~= 0);
                    numMatches = length(matchingTrials);
                else
                    % A cell has to have been found in a given trial
                    % AND be the correct context
                    
                    % These will be according to registered trials
                    matchingCellTrialIndices = find(cellTrialIndices ~= 0);
                    
                    % Now get the contexts for each of trials that where
                    % the cell was found to be present
                    matchingTrials = [];
                    for iCheck = matchingCellTrialIndices
                        if session.trial{iCheck}.context == context_id
                            matchingTrials = [matchingTrials; iCheck];
                        end
                    end
                ...
                if numMatches >= obj.config.analysis_cellreg_correlation_vs_orientation.minimium_trials_in_order_to_be_included
                    fprintf('Cell %d has %d matches to correlate. Found in trials\n', iCell, numMatches);
                    disp(matchingTrials)
                    ...
                    k = 0;
                    for iT = 1:length(matchingTrials)
                        iTrial = matchingTrials(iT);
                        % This is the registered cell's number for the
                        % given trial since it will chance trial to trial
                        trialCellNum = cellTrialIndices(iTrial);
                        disp(trialCellNum)
            ...
            if isempty(context_id)
                ...
                outputFile = sprintf('cellreg_correlation_vs_orientation_context_%d.mat', context_id);
            end
            ...
            if isempty(context_id)
                outputFile2 = sprintf('%s_cellreg_correlation_vs_orientation.png', session.name);
            else
                outputFile2 = sprintf('%s_cellreg_correlation_vs_orientation_context_%d.png', session.name, context_id);
            end
            saveas(h, fullfile(outputFolder, outputFile2), 'png')
            close(h)
