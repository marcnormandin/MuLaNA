...
plot(poly(:,2), poly(:,1), 'b-*', 'linewidth', 10)
hold on
plot([poly(1,2), poly(end,2)], [poly(1,1), poly(end,1)], 'b-*', 'linewidth', 10)
grid on
...
% for iSim = 1:numSimulations
    ...
    for i = 1:size(intersections,1)
       plot([Mv(2), intersections(i,2)], [Mv(1), intersections(i,1)], 'k-*')
       plot([Mv(2), intersectionsOp(i,2)], [Mv(1), intersectionsOp(i,1)], 'r-*')
    end
    %Plot the heading direction
    Rv = [cosd(mhdr), sind(mhdr)];
    b = 20;
    plot([Mv(2), Mv(2) + b*Rv(2)], [Mv(1), Mv(1) + b*Rv(1)], 'm-', 'linewidth', 5)
%end
axis equal
%axis square
% telapsed = toc(tstart);
